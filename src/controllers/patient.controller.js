...
class PatientController {
    ...
    createPatient = async (req, res, next) => {
        try {
            const { first_name, last_name, birth_date, address, phone, passport, area_id } = req.body;
    ...
    deletePatient = async (req, res, next) => {
        try {
            ...
            const selectedPatientImage_url = await this.#_patientModel.findById(patientId).select("image_url")
            ...
            fs.unlink(path.join(process.cwd(), "uploads", selectedPatientImage_url.image_url), (err) => {
                console.log(err)
            })
    ...
    deleteAllPatients = async (_, res, next) => {
        try {
            const allImagesUrl = await this.#_patientModel.find({}).select("image_url -_id")
    ...
    #_check_if_exists_document = async (id) => {
        const foundedPatient = await this.#_patientModel.findById(id).populate([
